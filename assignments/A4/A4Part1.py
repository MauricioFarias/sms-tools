...
def extractMainLobe(window, M):
    ...
    dftbuffer[:hM1] = wm[hM2:]
    dftbuffer[-hM2:] = wm[:hM2]
    X = fft(dftbuffer)
    ...
    mlVar = mXmainLobe[i]
    mVar = mX[i+1]
    while ((mlVar >= mVar) and (i < N)):
        mXmainLobe = np.append(mXmainLobe, np.array([mX[i+1]]))
        i += 1
        mlVar = mXmainLobe[i]
        mVar = mX[i+1]
    
    mXmainLobe = np.append(mX[-i:],mXmainLobe)
